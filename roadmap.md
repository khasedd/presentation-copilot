...
**Current status — baseline passed; exit work pending:** The foundation is verified offline and the initial live-ingestion/source-check portion passes. At the owner's explicit checkpoint, live mutation/reorder testing awaits approval and the overall exit gate remains open. The live source has one group with two child shapes, not a group-inside-group case. Concept extraction and broader representative edge-case evaluation are not complete.
